...
public class ServerImpl extends UnicastRemoteObject implements Server
{
    ...
    @Override
    public void recieveProbe(ArrayList<Integer> visited, int id) throws RemoteException {
        if(this.activeTransaction != null) {
            if(this.activeTransaction.getWaitingForResource() != null) {
                // if deadlock
                if(visited.contains(this.serverId)) {
                    visited.add(this.serverId);
                    println("Deadlock", this.activeTransaction.getId());
                    println(visited.toString(), this.activeTransaction.getId());
                    resources.get(id).abortCurrentTransaction(this.activeTransaction);
                }
            } else {
                visited.add(this.serverId);
                println(visited.toString(), this.activeTransaction.getId());
                if(this.activeTransaction.getWaitingForResource() != null) {
                    // send it on
                    this.activeTransaction.getWaitingForResource().server.recieveProbe(visited, id);
                }
            }
        }
    }
